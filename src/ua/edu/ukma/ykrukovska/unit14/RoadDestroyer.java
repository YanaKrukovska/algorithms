...
public class RoadDestroyer {
    ...
    private static int findMinPrice(int currentCity, int[][] cities) {
        ...
        do {
            ...
            x = find(0, currentCity, 999999999, visited, citiesCopy);
            price += x;
        } while (x != 0 && price != 0);
        return price;
    }

    private static int find(int x, int t, int roadCurrent, boolean visited[], int result[][]) {
        if (x == t) {
            return roadCurrent;
        }
        if (visited[x]) {
        ...
        for (int road, y = 0; y < result.length; ++y) {
            if (result[x][y] > 0 && (road = find(y, t, Math.min(roadCurrent, result[x][y]), visited, result)) > 0) {
                result[x][y] -= road;
                result[y][x] += road;
                return road;
            }
